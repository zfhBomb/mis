define(function (require, exports, module) {
    var ieRange=!typeof document.createRange === 'function';
    ...
    module.exports={
        ...
        // 将当前选区折叠
        collapseRange : function(range, opt) {
        ...
        // 获取选区的文字
        getRangeText:ieRange ? emptyFn : function (range) {
            range = range || this.currentRange();
            if (!range) {
                return;
            }
        ...
        // 获取选区对应的DOM对象
        getRangeElem:ieRange ? emptyFn : function (range) {
            range = range || this.currentRange();
        ...
        // 选区内容是否为空？
        isRangeEmpty:ieRange ? emptyFn : function (range) {
            range = range || this.currentRange();
        ...
        // 保存选区数据
        saveSelection:ieRange ? emptyFn : function (range) {
            var self = this,
                _parentElem,
                selection,
                txt = self.textEditor.get(0);
        ...
        // 恢复选中区域
        restoreSelection:ieRange ? emptyFn : function (range) {
            var selection;
        ...
        // 根据elem恢复选区
        restoreSelectionByElem:ieRange ? emptyFn : function (elem, opt) {
        ...
        // 初始化选区
        initSelection:ieRange ? emptyFn : function () {
            var editor = this;
        ...
        // 根据元素创建选区
        setRangeByElem : ieRange ? emptyFn : function (elem) {
            var editor = this;
            var txtElem = editor.textEditor.get(0);
